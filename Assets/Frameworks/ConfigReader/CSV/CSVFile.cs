...
namespace Frameworks.CSV
{
	...
	public class CSVFile
	{
		...
		public static CSVFile CreateCSVFile(string strPath)
		{
			...
			CSVFile newCSVFile = CreateCSVFileFromMemory(textAsset.text);
		...
		public static CSVFile CreateCSVFileByStream(string strPath)
		{
			string textAsset = "";
			...
			{
				Log.Print(LogLevel.Error, "Read File Path:" + strPath + " Error, Exception:" + e.Message + " Stack:" + e.StackTrace);
				return null;
			}
#else
			Log.Print( LogLevel.Error, "This function is not support on webplayer");
#endif

			CSVFile newCSVFile = CreateCSVFileFromMemory(textAsset);
		...
		public static CSVFile CreateCSVFileFromMemory(string textData)
		{
			CSVBase csvBase = CSVBase.CreateCSV(textData);

			CSVFile newCSVFile = new CSVFile();

			newCSVFile.m_Path = "";

			newCSVFile.Elem = new string[csvBase.LineData.Count][];

			for (int i = 0; i < csvBase.LineData.Count; i++)
			{
				newCSVFile.Elem[i] = csvBase.LineData[i].colData.ToArray();
			}
		...
		public static CSVFile CreateCSVFileFromMemory(byte[] byteData)
		{
