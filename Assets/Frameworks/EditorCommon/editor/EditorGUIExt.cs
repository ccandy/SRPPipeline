...
namespace Frameworks
{
	static class EditorGUIExt
	{
		...
		public static void OnAssetRefGUILayout(AssetRef assetRef, string name)
		{
			if (assetRef == null)
				return;
			assetRef.Path = EditorGUILayout.TextField(name, assetRef.Path);
		}
		...
		public static void OnFieldGUILayout(ref System.Object obj, Type type, string name)
		{
			...
			else if (type == typeof(AssetRef))
			{
				if (obj == null)
				{
					obj = new AssetRef(typeof(UnityEngine.Object));
				}
				OnAssetRefGUILayout((AssetRef)obj, name);
			}
			...
			{
				EditorArray.OnGUIList( (System.Collections.IList)obj, type, name);
			}
			else
			{
		...
		public static void OnClassFieldGUILayout(ref System.Object e, string name)
		{
			...
			for (int i = 0; i < fields.Length; ++i)
			{
				...
				else if (field.FieldType == typeof(AssetRef) )
				{
					if (fieldObject == null)
					{
						field.SetValue(e, new AssetRef(typeof(UnityEngine.Object)));
					}
					OnAssetRefGUILayout((AssetRef)fieldObject, field.Name);
				}
				...
				else if (field.FieldType.GetInterfaces().Contains(typeof(System.Collections.IList)))
				{
					EditorArray.OnGUIList((System.Collections.IList)fieldObject, field.FieldType, field.Name);
				}
				else
				{
