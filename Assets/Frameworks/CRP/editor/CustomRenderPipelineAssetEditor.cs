...
namespace Frameworks.CRP
{
	...
	public class CustomRenderPipelineAssetEditor : Editor
	{
		...
		SavedBool	m_isCubeSH_HDR_To_LDR;
		SavedFloat	m_CubeSH_Blur;

		SerializedProperty m_ShadowTypeProp;
		SerializedProperty m_ShadowmapResolutionProp;

		SerializedProperty m_ShadowDistanceProp;
		SerializedProperty m_ShadowCascadeCountProp;
		SerializedProperty m_ShadowCascadeSplitsProp;
		...
		SerializedProperty	m_RenderPassBlockAssetsProp;
		...
		void OnEnable()
		{
			...
			m_isCubeSH_HDR_To_LDR	= new SavedBool($"{target.GetType()}.isCubeSH_HDR_To_LDR", false);
			m_CubeSH_Blur			= new SavedFloat($"{target.GetType()}.CubeSH_Blur", 1.0f);

			m_RenderPassBlockAssetsProp = serializedObject.FindProperty("renderPassBlockAssets");
			m_RenderPassBlockAssetsList = new ReorderableList(serializedObject, m_RenderPassBlockAssetsProp, true, true, true, true);

			DrawRendererListLayout(m_RenderPassBlockAssetsList, m_RenderPassBlockAssetsProp);



			m_HDR = serializedObject.FindProperty("SupportsHDR");
			m_MSAA = serializedObject.FindProperty("UseMSAA");
			m_RenderScale = serializedObject.FindProperty("RenderScale");

			m_ShadowTypeProp = serializedObject.FindProperty("shadowType");
			m_ShadowmapResolutionProp = serializedObject.FindProperty("ShadowMapResolution");

			m_ShadowDistanceProp = serializedObject.FindProperty("ShadowDistance");

			m_ShadowCascadeCountProp = serializedObject.FindProperty("ShadowCascadeCount");
			m_ShadowCascadeSplitsProp = serializedObject.FindProperty("CascadeSplits");
			...
			m_SRPBatcher = serializedObject.FindProperty("UseSRPBatcher");
			m_SupportsDynamicBatching = serializedObject.FindProperty("SupportsDynamicBatching");
			m_MixedLightingSupportedProp = serializedObject.FindProperty("MixedLightingSupported");
		...
		void DrawRendererListLayout(ReorderableList list, SerializedProperty prop)
		{
			...
			list.onRemoveCallback = li =>
			{
				...
				//if (li.index != m_RenderPassBlockAssetsProp.intValue)
				//{
		...
		void DrawGeneralSettings()
		{
			...
			if (m_GeneralSettingsFoldout.value)
			{
				...
				if (asset.renderPassBlockAssets.Count <= 0)
		...
		void DrawQualitySettings()
		{
			...
			if (m_QualitySettingsFoldout.value)
			{
				...
				m_RenderScale.floatValue = EditorGUILayout.Slider(Styles.renderScaleText, m_RenderScale.floatValue, CustomRenderPipeline.minRenderScale, CustomRenderPipeline.maxRenderScale);
				EditorGUI.indentLevel--;
				EditorGUILayout.Space();
				EditorGUILayout.Space();
			}
		...
		void DrawLightingSettings()
		{
			...
			if (m_LightingSettingsFoldout.value)
			{
				...
				// Environment
				asset.EnvironmentType = (EnvType)EditorGUILayout.EnumPopup("Environment Type:", asset.EnvironmentType);
				if (asset.EnvironmentType == EnvType.GLOBAL_CUBE)
				{
					asset.GlobalCubeMap	  = (Cubemap)EditorGUILayout.ObjectField("GlobalCubeMap", asset.GlobalCubeMap, typeof(Cubemap), false);
					if (asset.GlobalCubeMap != null)
					{
						EditorGUI.indentLevel++;
						m_isCubeSH_HDR_To_LDR.value = EditorGUILayout.Toggle("isConverHDRToLDR", m_isCubeSH_HDR_To_LDR.value);

						m_CubeSH_Blur.value = EditorGUILayout.FloatField("CubeSH Blur", m_CubeSH_Blur.value);

						if (GUILayout.Button("Create SH"))
						{
							LightSHBuilder.BuildSH(ref asset.CubeSH9, asset.GlobalCubeMap, m_isCubeSH_HDR_To_LDR.value, m_CubeSH_Blur.value);
						}
				...
				EditorGUI.indentLevel++;
				EditorGUI.BeginDisabledGroup(disableGroup);
				EditorGUILayout.PropertyField(m_ShadowTypeProp, Styles.supportsMainLightShadowsText);
				EditorGUI.EndDisabledGroup();

				disableGroup |= !m_ShadowTypeProp.boolValue;
				EditorGUI.BeginDisabledGroup(disableGroup);
				EditorGUILayout.PropertyField(m_ShadowmapResolutionProp, Styles.mainLightShadowmapResolutionText);
				EditorGUI.EndDisabledGroup();
		...
		void DrawShadowSettings()
		{
			...
			if (m_ShadowSettingsFoldout.value)
			{
				...
				EditorGUILayout.IntSlider(m_ShadowCascadeCountProp, CustomRenderPipelineAsset._ShadowCascadeMinCount, CustomRenderPipelineAsset._ShadowCascadeMaxCount, Styles.shadowCascadesText);

				if (EditorGUI.EndChangeCheck())
				{
					if (asset.ShadowCascadeCount == 1)
					{
						asset.CascadeSplits = Vector4.zero;
					}
					else if (asset.ShadowCascadeCount == 2)
					{
						asset.CascadeSplits = new Vector4(CustomRenderPipelineAsset.DefaultCascade2Split,0,0,0);
					}
					else if (asset.ShadowCascadeCount == 3)
					{
						asset.CascadeSplits = new Vector4(CustomRenderPipelineAsset.DefaultCascade3Split.x, CustomRenderPipelineAsset.DefaultCascade3Split.y,0,0);
					}
					else if (asset.ShadowCascadeCount == 4)
					{
						asset.CascadeSplits = CustomRenderPipelineAsset.DefaultCascade4Split;
					}
				}
				...
				{
					asset.CascadeSplits[i] = EditorGUILayout.Slider(asset.CascadeSplits[i], 0.0f, 1.0f - sum);
					sum += asset.CascadeSplits[i];
				}

				EditorGUIExt.OnShadowCascadeSliderPercentGUILayout( ref asset.ShadowCascadeCount, ref asset.CascadeSplits, asset.ShadowDistance);

				m_ShadowDepthBiasProp.floatValue = EditorGUILayout.Slider(Styles.shadowDepthBias, m_ShadowDepthBiasProp.floatValue, 0.0f, CustomRenderPipeline.maxShadowBias);
				m_ShadowNormalBiasProp.floatValue = EditorGUILayout.Slider(Styles.shadowNormalBias, m_ShadowNormalBiasProp.floatValue, 0.0f, CustomRenderPipeline.maxShadowBias);
				EditorGUI.indentLevel--;
				EditorGUILayout.Space();
				EditorGUILayout.Space();
			}
